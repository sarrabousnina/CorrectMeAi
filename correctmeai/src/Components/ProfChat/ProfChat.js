...
import "./ProfChat.css";
import robotIcon from "../../Images/robot.png"; // mascot image
...
export default function ProfChat({ sessionId = "prof-global" }) {
    ...
    const send = async (text) => {
        ...
        try {
            // 1) Try the agent (DB-aware tools) first
            ...
            if (agent.handled) {
                // Agent executed a tool and produced a final answer
                setMessages((m) => [...m, { role: "assistant", text: agent.reply }]);
                setBusy(false);
                return;
            }

            // 2) Not handled by agent → fall back to streaming chat (with RAG)
            ...
            // Prepare an empty assistant bubble for streaming tokens
    ...
    const ChatShell = (
        <div className="pcb-shell">
            ...
            <form className="pcb-inputbar" onSubmit={onSubmit}>
                ...
                />
                <button className="pcb-send" disabled={busy}>
                    Send
                </button>
            </form>
        </div>
