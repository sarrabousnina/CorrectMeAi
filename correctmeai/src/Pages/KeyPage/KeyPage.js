...
const KeyPage = () => {
    ...
    const handleMCQInput = (qid, val) => {
        setSelectedChoices((prev) => ({ ...prev, [qid]: val }));
    };
    ...
    const renderTextBlock = (textBlock) => {
        ...
        return parts.map((part, idx) => {
            ...
            const exclude = excludedLabels.some((l) => before.includes(l));
            if (exclude) return <span key={`skip-${idx}`}>__________</span>;
        ...
        <div style={{ padding: 20, maxWidth: 900, margin: "0 auto" }}>
            ...
            <section style={{ marginBottom: 20 }}>
                ...
                >
                    {blocks.map((b, idx) =>
                        b.type === "text" ? (
                            <span key={idx}>{renderTextBlock(b.content)}</span>
                        ) : (
                            renderMCQBlock(b.content, `q${idx}`)
                        )
                    )}
                </div>
            </section>
